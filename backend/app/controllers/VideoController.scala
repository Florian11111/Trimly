...
class VideoController @Inject()(val controllerComponents: ControllerComponents)(implicit ec: ExecutionContext) extends BaseController {
  ...
  def upload: Action[MultipartFormData[TemporaryFile]] = Action.async(parse.multipartFormData) { request =>
    ...
    request.body.file("video").map { video =>
      ...
      val volume = request.body.dataParts.get("volume").flatMap(_.headOption).map(_.toDouble).getOrElse(1.0)

      println(s"Video file received: $filename")
      println(s"Start time: $startTime, End time: $endTime, Volume: $volume")
      ...
      if (startTime < 0) {
        ...
        val processedVideoPath = processVideo(tempPath.toString, startTime, endTime, volume)
  ...
  def processVideo(videoPath: String, startTimeMs: Long, endTimeMs: Long, volumeFactor: Double): Option[File] = {
    val randomSuffix = Random.alphanumeric.take(8).mkString
